...
function createPost($request_values)
	{
		...
		if (mysqli_num_rows($result) > 0) { 
			array_push($errors, "Već postoji post sa tim nazivom.");
		}
		...
		if (count($errors) == 0) {
			...
			if(mysqli_query($conn, $query)){ 
				$_SESSION['message'] = "Uspješno objavljen post.";
	...
	function updatePost($request_values)
	{
		...
		if (count($errors) == 0) {
			...
			$_SESSION['message'] = "Uspješno uređivanje posta.";
			header('location: posts.php');
			exit(0);
		}
